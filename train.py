...
for epoch in trange(opt.epoch):
    ...
    for i, train_data in tqdm(enumerate(train_loader, 0)):
        ...
        input_lengths = torch.tensor([len(l) for l in preds.permute(1, 0, 2)], dtype=torch.int64, device=device)
        target_lengths = torch.tensor(labels_length, device=device, dtype=torch.int64)
    ...
    if epoch_loss < best_loss:
        ...
        torch.save(net, f='./crnn/trained_weights/epoch_{}_epoch_loss{:.5f}.pt'.format(epoch, epoch_loss))
